...
class ApiRequestSender(object):
    ...
    def set_webhook(self, url, webhook_events=None, is_inline=False):
        ...
        result = self.post_request(
            endpoint=BotApiEndpoint.SET_WEBHOOK,
    ...
    def get_account_info(self):
        ...
        return self.post_request(
            endpoint=BotApiEndpoint.GET_ACCOUNT_INFO,
            payload=json.dumps(payload)
        )
    ...
    def post_request(self, endpoint, payload):
        try:
            ...
            self._logger.error(
                u"failed to post request to endpoint=%s, with payload=%s. error is: %s",
                endpoint, payload, traceback.format_exc()
            )
            raise e
        except Exception as ex:
            self._logger.error(
                u"unexpected Exception while trying to post request. error is: %s",
    ...
    def get_online_status(self, ids=None):
        ...
        result = self.post_request(
            endpoint=BotApiEndpoint.GET_ONLINE,
            payload=json.dumps(payload)
        )
    ...
    def get_user_details(self, user_id):
        ...
        result = self.post_request(
            endpoint=BotApiEndpoint.GET_USER_DETAILS,
            payload=json.dumps(payload))
